...
class FireboltIdentifierPreparer(PGIdentifierPreparer):
...
class FireboltCompiler(PGCompiler):
    ...
    def visit_cast(self, cast, **kwargs):
        if self.dialect.supports_cast:
            return super(FireboltCompiler, self).visit_cast(cast, **kwargs)
        else:
    ...
    def visit_column(self, column, add_to_result_map=None,
        ...
        return super(FireboltCompiler, self).visit_column(column,
                                                          add_to_result_map=add_to_result_map, include_table=False,
                                                          **kwargs)

    def render_literal_value(self, value, type_):
        value = super(FireboltCompiler, self).render_literal_value(value, type_)
    ...
    def limit_clause(self, select, **kw):
        ...
        if select._offset_clause is not None:
            ...
            if select._limit_clause is None:
                text += self.process(select.literal(-1))
            else:
                text += '0'
...
class FireboltExecutionContext(default.DefaultExecutionContext):
...
class FireboltDialect(default.DefaultDialect):
    ...
    execution_ctx_cls = FireboltExecutionContext
    ...
    def dbapi(cls):
        try:
            import sqlalchemy_firebolt.connector as connector
        except:
            import connector
        return connector
    ...
    def get_columns(self, connection, table_name, schema=None, **kw):
        ...
        for r in rows:
            ...
            if r.type.startswith("AggregateFunction"):
                ...
                # the type from firebolt will be
