...
#python3 ci/firebolt_ingest_data.py $USERNAME $PASSWORD $ENGINE_NAME $DB_NAME
echo "Table for unit test has been created"

# Call pytest files to run unit tests
echo "Running unit tests"
username=$USERNAME password=$PASSWORD engine_name=$ENGINE_NAME db_name=$DB_NAME pytest ./tests/test_fireboltconnector.py
